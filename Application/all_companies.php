...
<h1 align="center">Companies in alphabetical order:</h1>
<p>
<?php
...
// Listing table Companies
$query = 'SELECT DISTINCT * FROM Companies ORDER BY CompName';
$result = mysqli_query($dbcon, $query)
  or die('Show tables failed: ' . mysqli_error());


echo "<div align='center'>Click company names to find more!</div></br>";
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "<tr>
              <td><a href='one_company.php?company=" . urlencode($row["CompName"]) . "'>" . $row["CompName"] . "</a></td>
              <td>" . $row["FoundYear"] . "</td>
              </tr>";
    }
    echo "</table></table>";
